...
const ASSET_BASE="https://cdn.jsdelivr.net/gh/anshaneja5/skyline-run@0c41526d8d4130c37c15e219c2c3737fa8cb4ad8/public/assets/models/";
...
async function loadAsset(file){
  try{
    ...
    loadedCount++;$("#loadingText").textContent=loadedCount+" / "+assetFiles.length;
    templates.set(file,gltf.scene);
    return gltf.scene;
  }catch(err){
    console.warn("Model load failed:",file,err);
    loadedCount++;$("#loadingText").textContent=loadedCount+" / "+assetFiles.length;
    return null;
  }
}
...
function setHighlight(obj,on){
  obj.traverse(o=>{
    ...
    if(m.emissive){
      if(on){m.emissive.setHex(0x087da1);m.emissiveIntensity=.75}
      else{m.emissive.copy(o.userData.baseEmissive||new THREE.Color());m.emissiveIntensity=o.userData.baseEI||0}
    }
  });
}
function selectBuilding(obj){
  ...
  const d=obj.userData.building,s=d.stats;$("#bcName").textContent=d.name;$("#bcType").textContent=d.type;$("#bcDevices").textContent=s[0];$("#bcPeople").textContent=s[1];$("#bcEnergy").textContent=s[2];$("#bcAlarm").textContent=s[3];$("#buildingCard").classList.add("show");
}
...
renderer.domElement.addEventListener("dblclick",e=>{pickAt(e.clientX,e.clientY);if(selected)focusOn(selected)});
...
function focusOn(obj){
  ...
  const radius=Math.max(size.x,size.y,size.z),startPos=camera.position.clone(),startTarget=controls.target.clone();
  const dir=new THREE.Vector3(1,.72,1).normalize();const endPos=center.clone().add(dir.multiplyScalar(radius*2.35+12));const endTarget=center.clone().add(new THREE.Vector3(0,size.y*.15,0));
  const t0=performance.now();controls.autoRotate=false;$("#autoRotate").classList.remove("active");
  function step(now){const t=Math.min(1,(now-t0)/780),e=1-Math.pow(1-t,3);camera.position.lerpVectors(startPos,endPos,e);controls.target.lerpVectors(startTarget,endTarget,e);controls.update();if(t<1)requestAnimationFrame(step)}
  requestAnimationFrame(step);
}
function homeView(){
  const start=camera.position.clone(),st=controls.target.clone(),end=new THREE.Vector3(92,74,100),et=new THREE.Vector3(0,7,0),t0=performance.now();
  function step(now){const t=Math.min(1,(now-t0)/700),e=1-Math.pow(1-t,3);camera.position.lerpVectors(start,end,e);controls.target.lerpVectors(st,et,e);controls.update();if(t<1)requestAnimationFrame(step)}requestAnimationFrame(step);
}
...
document.addEventListener("keydown",e=>{if(e.key==="Escape"){modal.classList.remove("show");$("#buildingCard").classList.remove("show")}});
