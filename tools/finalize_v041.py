...
def ensure_photo_png():
    """Create a broadly compatible PNG copy from the existing unveiling photo."""
...
def audit_sources():
    ...
    if PHOTO.stat().st_size<1000 or PHOTO_PNG.stat().st_size<1000: raise SystemExit('V0.4.1 unveiling photo is invalid or too small.')
    with Image.open(PHOTO_PNG) as im:
        photo_info={'format':im.format,'width':im.width,'height':im.height,'mode':im.mode}
    return {'index_refs':local,'screen_ids':unique,'js_syntax':syntax,'photo_bytes':PHOTO.stat().st_size,'photo_png_bytes':PHOTO_PNG.stat().st_size,'photo_png':photo_info}
...
def build_standalone():
    ...
    photo64=base64.b64encode(png_bytes).decode('ascii')
    photo_uri='data:image/png;base64,'+photo64
    # Replace both the original JPG reference and the compatibility PNG path.
    html=html.replace('assets/opc-reveal.jpg',photo_uri).replace('assets/opc-reveal.png',photo_uri)
    # Validate the exact embedded data URI bytes before writing the final file.
...
def render_contact_sheet(html):
    ...
    for i in range(1,21):
        shot=temp/f'screen-{i:02d}.png'; cmd=[chrome,'--headless=new','--disable-gpu','--no-sandbox','--hide-scrollbars','--window-size=1920,1080','--force-device-scale-factor=1','--virtual-time-budget=1800',f'--screenshot={shot}',f'{uri}#{i:02d}']
...
def package(html,contact):
    ...
    audit={'generated_at_utc':time.strftime('%Y-%m-%dT%H:%M:%SZ',time.gmtime()),'standalone':{'path':str(html.relative_to(ROOT)),'bytes':html.stat().st_size,'sha256':sha256(html)},'zip':{'path':str(zp.relative_to(ROOT)),'bytes':zp.stat().st_size,'sha256':sha256(zp)},'contact_sheet':{'path':str(contact.relative_to(ROOT)),'bytes':contact.stat().st_size,'sha256':sha256(contact)},'screen02_photo':{'path':str(PHOTO_PNG.relative_to(ROOT)),'bytes':PHOTO_PNG.stat().st_size,'sha256':sha256(PHOTO_PNG),'standalone_mime':'image/png'}}
    (QA/'release-build-audit-v0.4.1.json').write_text(json.dumps(audit,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); return zp

def main():
    QA.mkdir(parents=True,exist_ok=True); ensure_photo_png(); require_files(); audit=audit_sources(); (QA/'source-reference-audit-v0.4.1.json').write_text(json.dumps(audit,ensure_ascii=False,indent=2)+'\n',encoding='utf-8'); html=build_standalone(); contact=render_contact_sheet(html); zp=package(html,contact); print(json.dumps({'standalone':str(html.relative_to(ROOT)),'standalone_sha256':sha256(html),'zip':str(zp.relative_to(ROOT)),'zip_sha256':sha256(zp),'contact_sheet':str(contact.relative_to(ROOT)),'screen02_photo':str(PHOTO_PNG.relative_to(ROOT))},ensure_ascii=False,indent=2))
